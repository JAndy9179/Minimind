...
def train_epoch(epoch, loader, iters, teacher_model, lm_config_student, start_step=0, wandb=None, alpha=0.0, temperature=1.0):
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if (step % args.save_interval == 0 or step == iters) and is_main_process():
            ...
            ckp = f'{args.save_dir}/{args.save_weight}_{lm_config_student.hidden_dim}{moe_suffix}.pth'
...
if __name__ == "__main__":
    # 模拟用moe模型蒸馏dense模型，也可以用更大teacher_hidden_dim模型蒸馏更小student_hidden_dim的
    ...
    lm_config_student = MiniMindConfig(hidden_dim=args.student_hidden_size, num_hidden_layers=args.student_num_layers, use_moe=bool(args.student_use_moe))
    lm_config_teacher = MiniMindConfig(hidden_dim=args.teacher_hidden_size, num_hidden_layers=args.teacher_num_layers, use_moe=bool(args.teacher_use_moe))
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"MiniMind-Distill-S{args.student_hidden_dim}T{args.teacher_hidden_dim}-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
