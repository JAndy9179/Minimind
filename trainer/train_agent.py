...
def execute_tool(name, args):
    ...
    # 设置超时中断
    try:
        # 若执行时间超过 1 秒, 则执行回调函数 timeout_handler（打断当前执行）
...
def rollout_single(
    ...
    # 每次采样进行多轮交互（最多允许 max_turns 轮 tool interaction）
    for turn in range(max_turns):
        ...
        completion_masks = rollout_result.completion_masks[0].bool().tolist()
        ...
        if len(new_ids) != len(new_logps): Logger(
            f"rollout token/logps length mismatch: {len(new_ids)} vs {len(new_logps)}")
        new_ids = [t for t, m in zip(new_ids, completion_masks) if m]
        new_logps = [lp for lp, m in zip(new_logps, completion_masks) if m]
        ...
        # 因为 tools 输出的 token 不是模型生成的, 因此不会参与模型更新的计算
    ...
    return (
        final_output,           # 最终回答（model response 的最后一条回答）
        final_context,          # 最终的完整上下文
        prompt_ids,             # 输入（编码）
        response_ids,           # 输出（编码，model response + tool result）
        response_mask,          # 输出（掩码，model response + tool result，其中 model response 部分为 1，tool result 部分为 0）
        response_old_logps,     # 输出（logps，model response + tool result，其中 model response 部分为对应 token 的 logps，tool result 部分为 0.0）
        list(all_outputs),      # 所有的 model response
        finished
    )
    ...
    for idx, response in enumerate(completions):
        ...
        if not tool_calls:
            ...
            if reward_model is not None:
                ...
                reward += score                                               # Reward Model 分
            reward -= rep_penalty(answer)
            rewards[idx] = max(min(reward, 3.0), -3.0)                        # 总分裁剪
        # -------- 有工具调用：执行结果奖励 --------
        else:
...
def rl_train_epoch(epoch, loader, iters, rollout_engine, ref_model, reward_model=None, start_step=0, wandb=None, use_sglang=False):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        for p, r, m, old_lp in zip(
            ...
            ids = p + r
            mask = [0] * len(p) + m
            old_logps = [0.0] * (len(p) - 1) + old_lp
            if len(ids) > args.max_total_len:
                ids = ids[-args.max_total_len:]
                mask = mask[-args.max_total_len:]
                old_logps = old_logps[-(len(ids) - 1):]
            # 返回 mask 中第一个为 1 的位置, 作为 prompt 的长度
            # 之所以能这么处理是因为 p, r, m, old_lp 都是一条样本对应的一维序列
            prompt_len = next(
                (i for i, v in enumerate(mask) if v == 1),  # iterator
                len(mask)                                   # default
            )
            packed_samples.append((ids, mask, prompt_len, old_logps))

        seq_lens = torch.tensor(
            [len(ids) for ids, _, _, _ in packed_samples],
            device=args.device
        )
        ...
        prompt_lens = torch.tensor(
            [prompt_len for _, _, prompt_len, _ in packed_samples],
            device=args.device
        )
        # 用于控制哪些 token 参与训练（loss 计算）
        ...
        eos_idx = torch.full(
            (completion_mask.size(0),), completion_mask.size(1) - 1,
        ...
        grouped_reward = rewards.view(-1, args.num_generations)
        mean_r = grouped_reward.mean(dim=-1).repeat_interleave(args.num_generations)
        std_r = grouped_reward.std(dim=-1, unbiased=False).repeat_interleave(args.num_generations)
        advantages = (rewards - mean_r) / (std_r + 1e-4)

        # ========== Loss 计算 ==========
        # advantages.shape = (B * num_gen,)
        # per_token_logps.shape = (B * num_gen, max_seq_len - 1)
        # ref_per_token_logps.shape = (B * num_gen, max_seq_len - 1)
        # old_per_token_logps.shape = (B * num_gen, max_seq_len - 1)
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            gs = grouped_reward.std(dim=1, unbiased=False).mean().item()
        ...
        del completions, rewards, grouped_reward, mean_r, std_r, advantages, completion_mask
...
if __name__ == "__main__":
    ...
    lm_config = MiniMindConfig(hidden_dim=args.hidden_size, num_hidden_layers=args.num_hidden_layers, use_moe=bool(args.use_moe))
    ...
    ref_model, _ = init_model(lm_config, args.from_resume, device=args.device)
    ...
    if args.use_compile == 1:
        model = torch.compile(actor_model)
        Logger('torch.compile enabled')
        rollout_engine.update_policy(model)
    if dist.is_initialized():
        model = DistributedDataParallel(actor_model, device_ids=[local_rank])
